...
class Mainform(Gtk.Window):
    def __init__(self):
            ...
            txtReadme="" + \
                "软件说明：\n" + \
                "此工具用于将扩展词库合并到当前输入法的词库文件中。\n" + \
                "比如医药行业中使用的特殊药品名称，都已有人整理好并做成扩展词库。\n" + \
                "\n" + \
                "待合并词库可以是：搜狗词库(.scel)，或者您自己创建的.txt1/.txt2明文格式词库\n" + \
                "输入法词库可以是：.mb、  .dict 后缀的2种格式(目前仅支持这两种)\n" + \
                "明文的词库可以是：.txt1、.txt2 后缀的2种格式(此类格式的文件用于格式中转、词库编辑)\n" + \
                "\n" + \
                "本软件支持的.txt1/.txt2明文格式，都是一行一条数据，行首带@字符表示拼音编码，具体如下：\n" + \
                ".txt1格式为：\"编码 词条\"\n" + \
                "　　　　　　　此格式可通过txt2mb工具转成.mb格式\n" + \
                ".txt2格式为：\"词条 编码 词频\"(编码带'分隔符)\n" + \
                "　　　　　　　此格式可通过libime_pinyindict/libime_tabledict工具转成.dict格式\n" + \
                "\n" + \
