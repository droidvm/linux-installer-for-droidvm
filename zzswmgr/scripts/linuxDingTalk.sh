...
function sw_download() {
	case "${CURRENT_VM_ARCH}" in
		"arm64")
			swUrl="https://dtapp-pub.dingtalk.com/dingtalk-desktop/xc_dingtalk_update/linux_deb/Release/com.alibabainc.dingtalk_7.1.0.31120_arm64.deb"
			download_file_axel "${DEB_PATH1}" "${swUrl}"
			exit_if_fail $? "下载失败，网址：${swUrl}"
		;;
		"amd64")
			swUrl="https://dtapp-pub.dingtalk.com/dingtalk-desktop/xc_dingtalk_update/linux_deb/Release/com.alibabainc.dingtalk_7.1.0.31120_amd64.deb"
			download_file_axel "${DEB_PATH1}" "${swUrl}"
			exit_if_fail $? "下载失败，网址：${swUrl}"
		;;
		*) exit_unsupport ;;
...
function sw_create_desktop_file() {
	...
	sed -i 's#\./com.alibabainc.dingtalk#./com.alibabainc.dingtalk --no-sandbox#g' /opt/apps/com.alibabainc.dingtalk/files/Elevator.sh

	echo "正在生成桌面文件"
	# tmpfile=${DIR_DESKTOP_FILES}/com.alibabainc.dingtalk.desktop
...
if [ "${action}" == "卸载" ]; then
	# echo "暂不支持卸载"
	# exit 1
	rm -rf ${app_dir} ${DIR_DESKTOP_FILES}/com.alibabainc.dingtalk.desktop
	rm2desktop com.alibabainc.dingtalk.desktop
else
	sw_download
	sw_install
