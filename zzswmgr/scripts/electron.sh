...
function sw_download() {
	...
	case "${CURRENT_VM_ARCH}" in
		"arm64")
			swUrl="https://cdn.npmmirror.com/binaries/electron/v${SWVER}/electron-v${SWVER}-linux-arm64.zip"
			download_file2 "${DEB_PATH1}" "${swUrl}"
			exit_if_fail $? "下载失败，网址：${swUrl}"
		;;
		"amd64")
			swUrl="https://cdn.npmmirror.com/binaries/electron/v${SWVER}/electron-v${SWVER}-linux-x64.zip"
			download_file2 "${DEB_PATH1}" "${swUrl}"
			exit_if_fail $? "下载失败，网址：${swUrl}"
		;;
		*) exit_unsupport ;;
...
function sw_install() {
	...
	apt-get install -y unzip libasound2
