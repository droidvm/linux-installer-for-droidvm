...
if [ "${action}" == "卸载" ]; then
	apt-get remove  -y engrampa
	exit_if_fail $? "卸载失败"
else
	apt-get install -y engrampa tar gzip zip unzip xz-utils unrar cpio
	exit_if_fail $? "安装失败"
...
/usr/share/engrampa/translations 中没有简体中文语言包:
https://translate.lxqt-project.org/projects/lxqt-desktop/engrampa/zh_CN/
...
github仓库上的源码，是包含了的！
https://github.com/lxqt/engrampa
