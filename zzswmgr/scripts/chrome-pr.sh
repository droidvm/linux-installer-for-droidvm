...
function sw_install() {
	apt-get install -y python3-pip python3.11-venv libasound2 # 下载60M，解压后200M
	...
	sudo -u ${ZZ_USER_NAME} ${ZZSWMGR_MAIN_DIR}/ezapp/zzllq/bin/python -m playwright install chromium
	exit_if_fail $? "无法通过 playwright 安装 chromium! "

	dir_tmp=`ls -a /home/${ZZ_USER_NAME}/.cache/ms-playwright/|grep chrom|tail -n 1`
...
function sw_create_desktop_file() {
	...
	cat <<- EOF > ${STARTUP_SCRIPT_FILE}
		#!/bin/bash
		exec /home/${ZZ_USER_NAME}/.cache/ms-playwright/${dir_tmp}/chrome-linux/chrome --no-sandbox \$@
	EOF
	...
	echo "Name=chrome-爬虫版"				>>${tmpfile}
	echo "Exec=${SWNAME} www.baidu.com"		>>${tmpfile}
	echo "Terminal=false"					>>${tmpfile}
	echo "Type=Application"					>>${tmpfile}
	echo "StartupWMClass=chrome"			>>${tmpfile}
	echo "Categories=Network;"				>>${tmpfile}
	echo "Comment=chrome"					>>${tmpfile}
	echo "Icon=/home/${ZZ_USER_NAME}/.cache/ms-playwright/${dir_tmp}/chrome-linux/product_logo_48.png"			>>${tmpfile}
	cp2desktop ${tmpfile}
