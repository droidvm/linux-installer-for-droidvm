...
# wget http://192.168.1.5:90/compile-libffi.sh -O ./scripts/compile-libffi.sh
# 
# 碰到的第一个问题，在这里有描述：https://github.com/libffi/libffi/issues/552     config.log: No such file or directory.
# 解法：打开 configure, 搜索这一串 "test -f $srcdir/config.log", 把mv指令换成cp指令即可
...
function sw_download() {
	...
	# swUrl=https://github.com/libffi/libffi.git
	swUrl=https://gitee.com/jdk-build-libs/libffi.git
	download_file3 "${SRC_DIR}" "${swUrl}"
	exit_if_fail $? "下载失败，网址：${swUrl}"

	# # 手动修改源文件后，生成补丁
	# diff -Npur ${SRC_DIR}/configure_old ${SRC_DIR}/configure> ${SWMGR_DIR}/scripts/res/${SWNAME}_1.patch

	# 重新下载过源码后，应该补丁
	patch ${SRC_DIR}/configure ${SWMGR_DIR}/scripts/res/${SWNAME}_1.patch
	exit_if_fail $? "补丁应用失败"
...
function sw_compile() {
	...
	do
		...
		export LDFLAGS=" -Wl,-rpath='\$\$ORIGIN/linuxlib' -latomic -llog -landroid -lc -lm " # -Wl,-Bstatic -ltalloc

		cd "${SRC_DIR}"
		./autogen.sh

		# CPP=${CXX} CXXCPP=${CXX} CPPFLAGS="-I${NDK_SYSROOT}/usr/include" \
		# ./configure --prefix="${INSTALL_DIR}" --host=aarch64-linux-android CFLAGS="--sysroot $NDK_SYSROOT"

		./configure --prefix="${INSTALL_DIR}" --host=aarch64-linux-android
