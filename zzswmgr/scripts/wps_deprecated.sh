...
function sw_install() {
	...
	sudo apt-get install -y libglu1-mesa libxslt-dev bsdmainutils
	...
	if [ ! -d /opt/kingsoft/wps-office ]; then
		...
		if [ -f /usr/share/applications/wps-office-wps.desktop ]; then
			...
			mv -f /usr/share/applications/wps-office-wps.desktop	/usr/share/applications/wps-office-wps-aarch64.desktop
			mv -f /usr/share/applications/wps-office-et.desktop		/usr/share/applications/wps-office-et-aarch64.desktop
			mv -f /usr/share/applications/wps-office-wpp.desktop	/usr/share/applications/wps-office-wpp-aarch64.desktop
		fi

		sudo dpkg -i ${DEB_PATH}
		# apt-get install -y ${ZZSWMGR_MAIN_DIR}/${DEB_PATH}
		if [ $? -ne 0 ]; then
	...
	# 加上这两行就出错？？？？？明天有空再看
	# cp -f ./tmp/deb-freetype-old/usr/lib/aarch64-linux-gnu/*  /opt/kingsoft/wps-office/office6/
	# exit_if_fail $? "旧版freetype复制失败"

	# cd /opt/kingsoft/wps-office/office6/ && ln -sf libfreetype.so.6 libfreetype.so.6.18.3
