...
function genscript_arm64() {
	cat <<- EOF > ${app_home}/tmp/dlwps.js
		function checkDLStatus() {
			dlcode = vmhelper.getDownloadStatus();
			console.log("dlcode: "+dlcode);
			switch(dlcode) {
			case 1:
				vmhelper.hide();
				break;
		...
		vmhelper.setDownloadFilename("wps.deb");
		document.querySelector("#banner_btn").click();	// 点击 立即下载
		links = document.querySelectorAll("a");			// 点击 deb arm64 版
		for (let link of links) {
			if(link.innerText === "For ARM") {			// "For ARM", "For X64"
				link.click();
				break;
			}
		}
		setTimeout("checkDLStatus()", 3*1000 );
	EOF
}
...
function genscript_amd64() {
	cat <<- EOF > ${app_home}/tmp/dlwps.js
		...
		links = document.querySelectorAll("a");			// 点击 deb arm64 版
		for (let link of links) {
			if(link.innerText === "For X64") {			// "For ARM", "For X64"
				link.click();
				break;
			}
		}
		...
		if [ ! -f ${DEB_PATH} ]; then
			...
			case "$?" in
				...
				"1")
					...
					# 启动内置浏览器
					zzllq https://linux.wps.cn dlwps.js
					if [ $? -ne 0 ]; then
						rm -rf ${app_home}/tmp/wps.deb
						echo "wps安装包下载失败"
						false
						exit_if_fail $? "下载失败"
					fi
					mv -f ${app_home}/tmp/wps.deb  ${DEB_PATH}
					;;
				*) 
					echo "您已取消安装"
					exit 1
