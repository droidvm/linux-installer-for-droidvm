...
function sw_download() {
	...
	swUrl=https://gitee.com/yelam2022/androix-xcb-libxcb
...
function sw_compile() {
	...
	do
		...
		# else
			echo "正在autogen"
			./autogen.sh --host=aarch64-linux
			exit_if_fail $? "autogen失败"
			...
			echo "正在编译"
			make V=1 -f makefile distclean
			make V=1 -f makefile -j$(nproc) proot # CFLAGS="${CFLAGS}" LDFLAGS="${LDFLAGS}"
			exit_if_fail $? "编译失败"
		# fi
