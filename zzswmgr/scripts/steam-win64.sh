...
function sw_install() {
	case "${CURRENT_VM_ARCH}" in
		"arm64")
				sudo -u ${ZZ_USER_NAME} exec64 "${DEB_PATH}"
...
function sw_create_desktop_file() {
	case "${CURRENT_VM_ARCH}" in
		"arm64")
				rm2desktop steam-win64.desktop
				rm -rf /usr/share/applications/steam-win64.desktop

				echo '#!/bin/bash'									> ./tmp/steam-win64
				echo "cd ~/\".wine64/drive_c/Program Files/Steam\""	>>./tmp/steam-win64
				echo "exec exec64 steam.exe"						>>./tmp/steam-win64

				mv -f ./tmp/steam-win64  /usr/bin/
				chmod 755 /usr/bin/steam-win64

				tmpfile=${DIR_DESKTOP_FILES}/steam-win64.desktop
				...
				echo "Name=steam-win64"	>>${tmpfile}
				echo "Exec=steam-win64"	>> ${tmpfile}
				cp2desktop ${tmpfile}

				echo "steam-win64 安装完成"
